...
- 적용 결과는 매 그랩 시작마다 로그로 남습니다 — `Euresys[CAM1] 라인레이트 설정값(합친후):11049.7Hz 트리거목표(합치기전×1/2):5524.9Hz 카메라:5524.9Hz 적용주기:181.00us` (§6-2의 밴드 합산이 켜져 있으면 트리거는 절반 속도로 나갑니다)
...
- `EuresysGrabber.DUAL_BAND_COMBINE`(기본 `true`)로 켜져 있습니다. `false`로 바꾸면 M0 단일 밴드로 되돌아가며, 관련 배율(라인레이트 절반, raw 버퍼 2배)도 자동으로 원래대로(×1) 계산됩니다.
- 켜져 있으면: 밴드 둘 다 활성화 → 트리거(라인레이트)는 `SystemParam.CamLineRate1~4` 값의 **절반**으로만 나감 → 보드/스트림은 raw로 논리 높이의 **2배**를 받음(`SequenceLength`는 예외 — 보드 CIC "사이클 수" 기준이라 그대로 둠) → `GrabThreadProc`가 raw 청크를 세로 1/2로 합쳐(인접 두 줄 평균) 내보내므로 `ImageManager` 등 이후 파이프라인은 이 사실을 몰라도 됩니다.
