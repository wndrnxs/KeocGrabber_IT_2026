...
- **레시피 값**입니다 — `RecipeParam.CamLinePeriod1~4`(us). 노광/게인과 똑같이 Setup 화면 **CAM SETTING** 탭의 각 카메라 패널에 `Line Period` 항목으로 편집하고, **Save를 눌러야 레시피 파일에 저장**됩니다(자동 저장 없음). 태그가 없는 구버전 레시피는 기본값 `90.5us`로 로드됩니다. 툴팁에 환산된 Hz가 같이 표시됩니다(로그와 eGrabber Studio는 Hz 표기).
- 입력은 **Enter 또는 포커스 이동 시** 화면에 반영됩니다. 키 입력마다 반영하면 "90.5"를 치는 도중 "9"에서 아래의 노출시간 연동이 걸려 노출이 9us로 잘려 버리기 때문입니다.
- 카메라에는 노광/게인처럼 실시간으로 들어가지 않고, `EuresysGrabber.fn_SetExternalTrigger()`가 그랩을 시작할 때마다 현재 레시피(`G.CURRRECIPE`)에서 읽어 적용합니다. 즉 **Save(현재 레시피 파일) → Main → Manual Grab** 순서로 확인합니다. Setup 화면의 라이브뷰(`Grab` 버튼)는 FreeRun 모드라 이 트리거 경로를 타지 않아 변경 효과가 보이지 않습니다.
- **노출시간 연동**: 카메라가 라인 1줄을 내는 데는 노출 외에 몇 us의 오버헤드가 더 들어, `노출 + 오버헤드 > 라인주기`가 되면 카메라가 `AcquisitionLineRate`를 스스로 깎고, 보드는 그 속도에 맞춰 느려져 **세로가 눌립니다**(실측: 노출 90.5us에서 카메라 10564.5Hz=94.66us → 오버헤드 4.2us, 보드 적용주기 97.5us → 세로 7% 눌림). 이 오버헤드는 모드마다 달라(프리런에선 7.8us) 상수로 두지 않고, 트리거 촬상 시작 시 `EuresysGrabber.fn_SetExternalTrigger()`가 readback으로 실측합니다: 라인레이트를 넣고 읽어서 목표보다 낮으면 `오버헤드 = 1e6/readback - 노출`을 구해 노출을 `라인주기 - 오버헤드 - 0.5us`로 줄이고 재시도(최대 3회). 로그 `카메라 10564.5Hz < 목표 11049.7Hz → 오버헤드 실측 4.16us, 노출 90.5->85.8us 재시도`. 실측된 값은 그 이후 `fn_SetExposureTime()`의 상한(`라인주기 - 오버헤드`)에 반영되어, UI/레시피에서 어떤 값이 오든 카메라에는 그 이하로만 들어갑니다(로그 `노출시간 90.5->85.8us 제한`). **Setup 화면의 Exposure 슬라이더 상한은 작업자가 이해하기 쉽게 라인주기 그대로**이고(오버헤드를 빼지 않음), 툴팁에 "카메라에는 실측 오버헤드 x.xx us를 뺀 값까지 적용"으로만 알려줍니다 — 즉 화면 90.5 / 카메라 85.8이 정상 상태입니다. 라인주기를 줄여 기존 노출이 상한을 넘게 되면 Setup 화면에서 그 자리에서 내려 카메라에 반영합니다. Matrox는 라인주기가 DCF 안에 있어 앱이 값을 모르므로 기존 고정 범위(0~255us)를 유지합니다.
- 적용 결과는 매 그랩 시작마다 로그로 남습니다 — `Euresys[CAM1] 라인레이트 목표:11049.7Hz 카메라:11049.7Hz 적용주기:90.50us (밴드합산 ON, raw 2배 수신 후 합산)`. 재시도 후에도 `카메라:`가 `목표:`보다 낮으면 그 라인주기는 카메라 한계(노출과 무관)라 보드가 카메라 속도×1.03으로 맞춥니다 — 이때는 비율이 틀어지므로 라인주기를 늘려야 합니다. **이 값은 §6-2의 밴드 합산 여부와 무관하게 그대로 씁니다** — 합산은 raw 줄 수만 조정할 뿐 사이클(=실제 이동거리 1칸)당 속도 자체는 바꾸지 않습니다.
- 물체가 늘어지거나 눌려 보이면(정사각 비율 안 맞음), 정사각형 물체를 찍어 결과 이미지의 H/W 비율을 재고 `현재 라인주기 × (H/W 비율)`로 보정값을 계산해 넣으면 됩니다.
- 이전 버전에서 `ImageGrabber.xml`에 있던 `<CamLineRate1~4>`(Hz)/`<CamLinePeriod1~4>`(us) 태그는 더 이상 읽지 않습니다 — 값을 바꿔 쓰던 현장은 레시피 파일에 다시 입력(Setup에서 편집 후 Save)해야 합니다.
...
- **레시피**: `C:/KEOC/Recipe/<RecipeName>.xml`. 카메라별 노광/게인/라인주기(최대 4채널, 라인주기는 §6-1), 상/하부 조명값, Crop ROI 테이블(2계열 Master 분할 촬상 시 이미지당 2개 ROI) 포함.
- **Setup 화면과 레시피의 관계** — 레시피 파일이 항상 기준이고 Setup 화면은 편집기입니다.
  - Setup에 **들어올 때** 현재 레시피(`G.CURRRECIPE`)의 노광/게인/조명/ROI 값을 화면에 띄웁니다(`[Load Curr Recipe]` 버튼과 동일 동작).
  - 화면에서 값을 바꾸면 카메라/조명에 **실시간 반영**됩니다(눈으로 보면서 맞추는 용도).
  - **Save**로 파일에 저장해야 남습니다. 저장한 파일이 지금 쓰는 레시피(`SystemParam.CurrRecipePath`)와 같으면 메모리의 현재 레시피도 같이 갱신됩니다.
  - **Main으로 돌아갈 때** 현재 레시피 값을 카메라/조명에 다시 적용합니다 — 저장하지 않은 실시간 변경은 버려지고, Main은 항상 레시피 값으로 촬상합니다.
...
- 카메라별 `CamLinePeriod1~4`는 Setup 화면에서 값을 바꿔도 그 자리에서 결과를 볼 수 없음(라이브뷰가 FreeRun이라 트리거 경로를 안 탐) — Save 후 Main에서 실제 촬상(Manual Grab 등)으로만 확인 가능(§6-1 참고)
